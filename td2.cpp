...
Acteur* trouverActeur(ListeFilms listeFilms, string nom) {
	Acteur* acteurTrouve = new Acteur;
...
Acteur* lireActeur(ListeFilms list, istream& fichier)
{
	...
	acteur->joueDans.elements = new Film*;
...
void detruireFilm(ListeFilms& listeFilms, string film) {
	int temp = 0;
	for (int i : range(listeFilms.nElements)) {
		if (listeFilms.elements[i]->titre == film) {
			for (int j : range(listeFilms.elements[i]->acteurs.nElements)) {
				/*if (listeFilms.elements[i]->acteurs.nElements == 1) {
					...
					for (int k = 0; k< listeFilms.elements[i]->acteurs.elements[j]->joueDans.nElements;k++)
					{
						if (listeFilms.elements[i]->acteurs.elements[j]->joueDans.elements[k]->titre == film) {
